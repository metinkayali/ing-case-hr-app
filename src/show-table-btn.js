...
import { buttonCss, ListViewType, ReduxStateElement } from './lib'

export class ShowTableBtn extends ReduxStateElement {
  static get styles() {
    return [
    buttonCss,
css`
...
`,
  ...
  stateChanged(state) {
    this.selectedClass = state.listViewType === ListViewType.Table ? 'selected' : ''
    this.update()
  }
  render() {
    return html`
  ...
  _onClick() {
    this.dispatch(setListView({ viewType: ListViewType.Table }))
  }
}
customElements.define('ing-case-show-table-btn', ShowTableBtn);
